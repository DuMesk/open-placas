...
O domínio oficial é `https://openplacas.com.br`. A configuração compartilhada em `lib/seo.ts` alimenta os metadados, links canônicos, Open Graph, `robots.txt` e `sitemap.xml`. As três páginas públicas estão incluídas no sitemap.
...
- WhatsApp: substituir `WHATSAPP_NUMBER` em `lib/contact.ts` quando o número definitivo estiver disponível. O placeholder `5561000000000` foi mantido em todos os links.
...
Envie `https://openplacas.com.br/sitemap.xml` na seção Sitemaps e inspecione as três URLs públicas. Consulte a [documentação oficial de sitemaps](https://support.google.com/webmasters/answer/7451001?hl=pt-BR). Nenhuma conta foi vinculada e nenhum sitemap foi enviado automaticamente.

Na hospedagem, configure HTTPS e redirecionamento permanente de HTTP e de `www.openplacas.com.br` para `https://openplacas.com.br`, se o subdomínio for utilizado. Verifique que o ambiente de produção não injeta `noindex` nem exige autenticação. Ambientes de preview devem ter proteção contra indexação configurada na hospedagem.
